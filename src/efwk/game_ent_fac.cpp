...
uint64_t entity_factory::create_explosion(double x, double y) {
        ...
        std::vector<cmp::frame_def> frame_defs;
        ...
        // Sound components.
        // -----------------
        auto nqueue = std::make_shared<cmp::noise_queue>();

        // Fx components.
        // --------------
        std::shared_ptr<cmp::fx> fxs;
...
uint64_t entity_factory::create_debris(double x, double y,
        ...
        // TTL generation.
        std::uniform_real_distribution<double> ttl_dist(
                        cfg::real("gameplay_debris_ttl_min"),
                        cfg::real("gameplay_debris_ttl_max"));
        double ttl_time = ttl_dist(rnd::engine);

        // Random movement.
        std::bernoulli_distribution dir_dist;
        
        std::uniform_real_distribution<double> mv_dist(vmin, vmax);
        ...
        std::uniform_real_distribution<double> rot_dist(theta_min, theta_max);
        ...
        auto on_death = explode
                ? cmp::create_explosion_sequence_reaction(1,
                                ...
                                cfg::real("gfx_explosion_seq_min_delay"))
                : std::shared_ptr<cmp::reaction>();

        // Movement nodes.
        // ---------------
        auto movement_bounds = std::shared_ptr<cmp::bounds>();
...
uint64_t entity_factory::create_player_ship(double x, double y) {
        ...
        auto flash_flag = std::make_shared<bool>(false);
        ...
        auto appearance_flash = cmp::create_static_bmp(
                        _resman.get_bitmap(res::res_id::PLAYER_SHIP_FLASH));
        ...
        // Movement components.
        // --------------------
        auto life_bounds = std::shared_ptr<cmp::bounds>();
...
uint64_t entity_factory::create_light_fighter_dyn(double x, double y, std::shared_ptr<cmp::dynamics> dynamics) {
        ...
        auto appearance_flash = cmp::create_static_bmp(
                        _resman.get_bitmap(res::res_id::ENEMY_LIGHT_FIGHTER_FLASH));
        ...
        std::vector<std::shared_ptr<cmp::reaction>> reactions {
                cmp::create_debris_reaction(
        ...
        std::bernoulli_distribution drop_health(cfg::real("gameplay_lfighter_health_drop_dist"));
        if(drop_health(rnd::engine))
                reactions.push_back(cmp::create_health_drop_reaction());

        std:: bernoulli_distribution drop_battery(cfg::real("gameplay_lfighter_battery_drop_dist"));
        if(drop_battery(rnd::engine))
                reactions.push_back(cmp::create_battery_drop_reaction());

        std::bernoulli_distribution drop_bul_up(cfg::real("gameplay_lfighter_gun_up_drop_dist"));
        if(drop_bul_up(rnd::engine))
                reactions.push_back(cmp::create_bullet_upgrade_drop_reaction());

        std::bernoulli_distribution drop_mis_up(cfg::real("gameplay_lfighter_ml_up_drop_dist"));
...
uint64_t entity_factory::create_heavy_fighter_dyn(double x, double y, std::shared_ptr<cmp::dynamics> dynamics) {
        ...
        auto appearance_flash = cmp::create_static_bmp(
                        _resman.get_bitmap(res::res_id::ENEMY_HEAVY_FIGHTER_FLASH));
        ...
        std::bernoulli_distribution drop_health(cfg::real("gameplay_hfighter_health_drop_dist"));
        if(drop_health(rnd::engine)) reactions.push_back(cmp::create_health_drop_reaction());

        std::bernoulli_distribution drop_battery(cfg::real("gameplay_hfighter_battery_drop_dist"));
        if(drop_battery(rnd::engine))
                reactions.push_back(cmp::create_battery_drop_reaction());

        std::bernoulli_distribution drop_bul_up(cfg::real("gameplay_hfighter_gun_up_drop_dist"));
        if(drop_bul_up(rnd::engine))
                reactions.push_back(cmp::create_bullet_upgrade_drop_reaction());

        std::bernoulli_distribution drop_mis_up(cfg::real("gameplay_hfighter_ml_up_drop_dist"));
        if(drop_mis_up(rnd::engine))
                reactions.push_back(cmp::create_missile_upgrade_drop_reaction());
...
uint64_t entity_factory::create_light_bomber_dyn(double x, double y, std::shared_ptr<cmp::dynamics> dynamics) {
        ...
        auto appearance_flash = cmp::create_static_bmp(
                        _resman.get_bitmap(res::res_id::ENEMY_LIGHT_BOMBER_FLASH));
        ...
        std::bernoulli_distribution drop_health(cfg::real("gameplay_lbomber_health_drop_dist"));
        if(drop_health(rnd::engine)) reactions.push_back(cmp::create_health_drop_reaction());

        std::bernoulli_distribution drop_battery(cfg::real("gameplay_lbomber_battery_drop_dist"));
        if(drop_battery(rnd::engine))
                reactions.push_back(cmp::create_battery_drop_reaction());

        std::bernoulli_distribution drop_bul_up(cfg::real("gameplay_lbomber_gun_up_drop_dist"));
        if(drop_bul_up(rnd::engine))
                reactions.push_back(cmp::create_bullet_upgrade_drop_reaction());

        std::bernoulli_distribution drop_mis_up(cfg::real("gameplay_lbomber_ml_up_drop_dist"));
        if(drop_mis_up(rnd::engine))
                reactions.push_back(cmp::create_missile_upgrade_drop_reaction());
...
uint64_t entity_factory::create_heavy_bomber_dyn(double x, double y, std::shared_ptr<cmp::dynamics> dynamics) {
        ...
        auto appearance_flash = cmp::create_static_bmp(
                        _resman.get_bitmap(res::res_id::ENEMY_HEAVY_BOMBER_FLASH));
        ...
        std::bernoulli_distribution drop_health(cfg::real("gameplay_hbomber_health_drop_dist"));
        if(drop_health(rnd::engine)) reactions.push_back(cmp::create_health_drop_reaction());

        std::bernoulli_distribution drop_battery(cfg::real("gameplay_hbomber_battery_drop_dist"));
        if(drop_battery(rnd::engine))
                reactions.push_back(cmp::create_battery_drop_reaction());

        std::bernoulli_distribution drop_bul_up(cfg::real("gameplay_hbomber_gun_up_drop_dist"));
        if(drop_bul_up(rnd::engine))
                reactions.push_back(cmp::create_bullet_upgrade_drop_reaction());

        std::bernoulli_distribution drop_mis_up(cfg::real("gameplay_hbomber_ml_up_drop_dist"));
        if(drop_mis_up(rnd::engine))
                reactions.push_back(cmp::create_missile_upgrade_drop_reaction());
...
uint64_t entity_factory::create_common_pickup(
        ...
        std::uniform_real_distribution<double> mv_dist(
...
uint64_t entity_factory::create_bullet(
        ...
        switch(upgrade_lvl) {
        case 1:
                image = res::res_id::BULLET_1;
                break;
        case 2:
                image = res::res_id::BULLET_2;
                break;
        case 3:
                image = res::res_id::BULLET_3;
                break;
        case 4:
                image = res::res_id::BULLET_4;
                break;
        case 5:
                image = res::res_id::BULLET_5;
                break;
        default:
                std::cerr << "Level = " << (unsigned)upgrade_lvl << " :O";
                exit(1);
        }
