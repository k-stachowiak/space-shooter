...
namespace cmp {
    ...
    class complex_fx : public fx {
        vector<shared_ptr<fx>> _fxs;
    public:

        complex_fx(vector<shared_ptr<fx>> const& fxs) : _fxs(fxs) {}
    ...
    public:
        ...
        flash_when_hurt(shared_ptr<bool> flag, double cooldown_time)
        : _flag(flag)
        , _cd(cooldown_time)
    ...
    shared_ptr<fx> create_complex_fx(std::vector<std::shared_ptr<fx>> const& fxs) {
        return shared_ptr<fx>(new complex_fx(fxs));
    }
    ...
    shared_ptr<fx> create_flash_when_hurt(shared_ptr<bool> flag, double cooldown_time) {
        return shared_ptr<fx>(new flash_when_hurt(flag, cooldown_time));
    }
